...
import { ChevronDown, Circle, Search, Filter, GitBranch, X } from "lucide-react";
...
export function Header() {
  ...
  // ── Scenarios (only fetched on forecast page) ──
  ...
  const selCls = "h-7 text-xs border border-gray-200 rounded-md px-2 bg-white focus:outline-none focus:ring-1 focus:ring-brand-500 text-gray-600 max-w-[130px]";
    ...
    <>
      ...
      <header className="bg-white border-b border-gray-100 px-6 flex-shrink-0">
        ...
        <div className="h-14 flex items-center gap-3 min-w-0">
            ...
            <>
              ...
              <div className="relative flex-shrink-0">
                ...
                <input
                  ...
                  placeholder="Search…"
                  className="pl-6 pr-2 h-7 text-xs border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-brand-500 bg-white w-36"
                />
              </div>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <select value={scenario} onChange={(e) => setScenario(e.target.value)} className={selCls}>
                ...
                {TIME_PRESETS.map((t) => (
                  <button
                    ...
                    className={[
                      ...
                      timeRange === t.key ? "bg-white text-brand-700 shadow-sm" : "text-gray-500 hover:text-gray-700",
                    ].join(" ")}
                  >
                    {t.label}
                  </button>
              ...
              {/* Category */}
              <select value={category} onChange={(e) => setCategory(e.target.value)} className={`${selCls} flex-shrink-0`}>
              ...
              {/* Location */}
              <select value={location} onChange={(e) => setLocation(e.target.value)} className={`${selCls} flex-shrink-0`}>
              ...
              {hasAnyFilter && (
                <button
                  onClick={clearAll}
                  className="flex items-center gap-1 h-7 px-2 text-xs text-gray-400 hover:text-gray-600 rounded-md hover:bg-gray-50 border border-transparent hover:border-gray-200 transition-colors flex-shrink-0"
                >
          ...
          {/* ── Spacer pushes user section right ── */}
            ...
            <div className="flex items-center gap-1.5 text-xs text-gray-400 flex-shrink-0">
              <div className="flex -space-x-1">
                {presence.slice(0, 4).map((p: any) => (
                  <div key={p.userId} className="w-6 h-6 rounded-full bg-brand-500 border-2 border-white flex items-center justify-center text-white text-xs font-medium" title={p.name}>
        ...
        {/* Active filter chips — slim strip below header row, only on forecast page */}
        {hasAnyFilter && (
          <div className="flex flex-wrap items-center gap-1.5 pb-2 -mt-1">
            {scenario && flatScenarios.find(s => s.id === scenario) && (
              <FilterChip label={flatScenarios.find(s => s.id === scenario)!.name} onRemove={() => setScenario("")} color="brand" icon={<GitBranch className="w-3 h-3" />} />
            )}
            {timeRange !== "next_8w" && (
              <FilterChip label={TIME_PRESETS.find(t => t.key === timeRange)?.label ?? ""} onRemove={() => setTimeRange("next_8w")} />
            )}
            {category && <FilterChip label={category}       onRemove={() => setCategory("")} />}
            {location && <FilterChip label={location}       onRemove={() => setLocation("")} />}
            {statusF  && <FilterChip label={statusF}        onRemove={() => setStatusF("")}
...
function FilterChip({ label, onRemove, icon, color = "gray" }: {
  label: string; onRemove: () => void; icon?: React.ReactNode; color?: "gray"|"brand"|"red"|"blue"|"green";
}) {
  const cls = { gray:"bg-gray-100 text-gray-600 border-gray-200", brand:"bg-brand-50 text-brand-700 border-brand-200",
    red:"bg-red-50 text-red-700 border-red-200", blue:"bg-blue-50 text-blue-700 border-blue-200", green:"bg-green-50 text-green-700 border-green-200" }[color];
  return (
    <span className={`inline-flex items-center gap-1 border rounded-full px-2 py-0.5 text-xs font-medium ${cls}`}>
      {icon}{label}
      <button onClick={onRemove} className="ml-0.5 opacity-60 hover:opacity-100"><X className="w-2.5 h-2.5" /></button>
    </span>
  );
}
